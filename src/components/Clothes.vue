...
export default {
  ...
  methods: {
    ...
    addLayer(category,image,category2=''){
      ...
      if(this.gender==='Female'){
        if (category==='c_clothes') {
          layer.style.transform= 'translate(245px, 459px)'
        }
        if (category==='前发a_front') {
          layer.style.transform= 'translate(290px, 196px)'
        }
        if (category==='后发a_front') {
          layer.zIndex = 0
        ...
        if (category==='ny') {
          layer.style.transform= 'translate(281px, 487px)'
        }
      }else{
        if (category==='c_clothes') {
          layer.style.transform= 'translate(184px, 458px)'
        }
        if (category==='ny') {
          layer.style.transform= 'translate(249px, 489px)'
        }
        if (category==='前发a_front') {
          layer.style.transform= 'translate(290px, 164px)'
        }
        if (category==='后发a_front') {
          layer.zIndex = 0
          layer.style.transform= 'translate(253px, 160px)'
    ...
    save(){
      ...
      const layers=JSON.stringify(this.layerList);

      // TODO:NOTICE 这样传过来是个json对象 后端@RequestBody ModelRequest request 获取
      this.$http.post(this.$constant.baseURL + "/model/save", {
        usertoken,gender,layers
      }).then(response => {
                ...
                const saveData = JSON.stringify(
                  {
                    ...
                    // 本地获取
                    model:this.tree.d_model[this.gender],
                  }
              )
              localStorage.setItem('layerList',saveData)
    ...
    initData(){
      ...
      const usertoken=localStorage.getItem("usertoken")
      this.$http.post(this.$constant.baseURL + "/model/getModel",usertoken).then(response => {
            if(response.data.code==200){
